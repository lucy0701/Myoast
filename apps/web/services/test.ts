...
export const postMemberTestResultAPI = (testId: string, memberId: string, score: number[]) =>
  apiBe.post(`/v1/member-test-result/${testId}/${memberId}`, score, { headers });

export const getTestResultAPI = (testId: string, testResultId: string) =>
  apiBe(`v1/tests/test/test-result/${testId}/${testResultId}`, { headers });
